...
import { getProduct, UpdateProduct } from "../inventory.service";
...
const UpdateInventory = ({open,setOpen,data}:props) => {
    ...
    defaultValues: {
      name: data?.name || '',
      code: data?.code || '',
      type_id: data?.type_id.toString() || '0',
  ...
  const handleSubmitForm = (value: z.infer<typeof formSchema>) =>{
    setSubmit(true);
    dispatch(UpdateProduct({ id: data?.id, body: value }));
  ...
  useEffect(() => {
    if (data) {
      ...
      setPreviewSrc(env.img_url + data.image); // Assuming data.image is available
    }
  }, [data, form]);
  ...
  useEffect(()=>{
    const fetchImage = async () => {
      try {
        const response = await fetch(env.img_url + data.image);
            ...
            <div className="flex grow flex-col gap-3 text-slate-900">
              ...
              <Form {...form}>
                ...
                <form onSubmit={form.handleSubmit(handleSubmitForm)} className="space-y-4">
                  ...
                  <Button type="submit" className="bg-emerald-500 hover:bg-emerald-600 shadow-md">
                    Update
                  </Button>
